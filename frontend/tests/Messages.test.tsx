...
describe('Messages Component', () => {
  ...
  test('should display messages container', async () => {
    renderMessages();
    const container = await screen.findByTestId('messages-container', {}, { timeout: 5000 });
    expect(container).toBeInTheDocument();
  });

  test('should display messages', async () => {
    renderMessages();
    const messages = await screen.findAllByTestId(/^message-[0-9]+$/, {}, { timeout: 5000 });
    expect(messages).toHaveLength(2);
  });

  test('should display message content', async () => {
    renderMessages();
    const messageContent = await screen.findByTestId('message-content-1', {}, { timeout: 5000 });
    expect(messageContent).toHaveTextContent('Hello, this is a test message!');
  });

  test('should display user information', async () => {
    renderMessages();
    const userId = await screen.findByTestId('message-user-1', {}, { timeout: 5000 });
    expect(userId).toHaveTextContent('anonymous');
  });

  test('should display pagination controls', async () => {
    renderMessages();
    const pagination = await screen.findByTestId('pagination', {}, { timeout: 5000 });
  ...
  test('should handle pagination', async () => {
    renderMessages();
    const nextButton = await screen.findByTestId('next-page', {}, { timeout: 5000 });
